...
class CustomTrainingArguments():
    ...
    prompt_probability: float = field(default=0.7, metadata={"help": "Probability of using previous text (as prompt)."})
...
def main():
   ...
   # vectorized_datasets["train"] = datasets.distributed.split_dataset_by_node(vectorized_datasets["train"], rank=training_args.local_rank, world_size=training_args.world_size)
    ...
    # https://github.com/huggingface/transformers/pull/28687#issuecomment-1970980372
    model.generation_config.language = "<|sv|>"  # for Swedish
...
#        )
    ...
    def compute_metrics(pred):
        ...
        # # replace pred.label_ids where -100 with pad_token_id
        # if custom_args.node_id == 0 and custom_args.proc_id == 0:
        #     print(f"pred.label_ids before: {pred.label_ids}")
        pred.label_ids[pred.label_ids == -100] = inference_tokenizer.pad_token_id
        # if custom_args.node_id == 0 and custom_args.proc_id == 0:
        #     print(f"pred.label_ids after: {pred.label_ids}")
        ...
        # if custom_args.node_id == 0 and custom_args.proc_id == 0:
        #     print(f"pred_str: {pred_str[0:15]}")
        #     print(f"label_str: {label_str[0:15]}")
    ...
    data_collator = DataCollatorSpeechSeq2SeqWithBPEDropoutPadding(
        ...
        prompt_probability=custom_args.prompt_probability,
