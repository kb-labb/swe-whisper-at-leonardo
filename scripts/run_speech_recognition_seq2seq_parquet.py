...
def main():
    ...
    if custom_args.multi_dataset:
        ...
        if custom_args.smdb_dataset:
            smdb = load_dataset("parquet", data_files=custom_args.smdb_dataset)     
            list_of_datasets.append(smdb["train"])   
        if custom_args.svt_dataset:
            svt = load_dataset("parquet", data_files=custom_args.svt_dataset)
            list_of_datasets.append(svt["train"])
        if custom_args.youtube_dataset:
            youtube = load_dataset("parquet", data_files=custom_args.youtube_dataset)
            list_of_datasets.append(youtube["train"])
        if custom_args.common_voice_dataset:
            common_voice = load_dataset("parquet", data_files=custom_args.common_voice_dataset)
            list_of_datasets.append(common_voice)
        if custom_args.riksdag_dataset:
            riksdag = load_dataset("parquet", data_files=custom_args.riksdag_dataset)
            list_of_datasets.append(riksdag)
        if custom_args.nst_dataset:
            nst = load_dataset("parquet", data_files=custom_args.nst_dataset)
            list_of_datasets.append(nst) 
        if custom_args.fleurs_dataset:
            fleurs = load_dataset("parquet", data_files=custom_args.fleurs_dataset)
            list_of_datasets.append(fleurs)
        if custom_args.swedia_dataset:
            swedia = load_dataset("parquet", data_files=custom_args.swedia_dataset)
            list_of_datasets.append(swedia)  
        
        all_datasets = interleave_datasets(list_of_datasets, probabilities=probabilities, seed=training_args.seed)
    ...
    def prepare_dataset(batch):
        ...
        inputs = feature_extractor(
            batch["audio_tensor"], sampling_rate=data_args.sampling_rate, return_attention_mask=forward_attention_mask
        )
        # process audio length
        batch[model_input_name] = inputs.get(model_input_name)[0]
        batch["input_length"] = len(batch["audio_tensor"])
        if forward_attention_mask:
            batch["attention_mask"] = inputs.get("attention_mask")[0]

        # process targets
        input_str = batch[text_column_name].replace('\n', ' ').lower() if do_lower_case else batch[text_column_name].replace('\n', ' ')
